...
Public Class FrmIngresos
    ...
    Private Sub FrmIngresos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        TxtMonto.Text = FormatCurrency(0, 2)
        LlenarGrilla()
    End Sub
    ...
    Sub SumarTotal()
        ...
        Sql = "SELECT * FROM TABLAINGRESOS WHERE FECHA BETWEEN @inicio AND @final"
        cmd.CommandText = Sql
    ...
    Sub LlenarGrilla()
        ...
        Dim Sql As String = "SELECT * FROM TABLAINGRESOS WHERE FECHA BETWEEN @FechaInicio AND @FechaFinal"
        Dim GastosDA As New MySqlDataAdapter(Sql, conn)
        GastosDA.SelectCommand.Parameters.AddWithValue("@FechaInicio", DTPInicio.Value)
        GastosDA.SelectCommand.Parameters.AddWithValue("@FechaFinal", DTPFinal.Value)
        Dim GastosDS As New DataSet
        GastosDA.Fill(GastosDS)
        GrillaGastos.DataSource = GastosDS.Tables(0)

        For Each fila As DataRow In GastosDS.Tables(0).Rows
            Monto += CDec(fila("MONTO"))
        Next
        LblMontoTotal.Text = FormatCurrency(Monto, 2)
    ...
    Private Sub btnguardar_Click(sender As Object, e As EventArgs) Handles btnguardar.Click
        ...
        Sql = "INSERT INTO TABLAINGRESOS (MOTIVO, MONTO, OBSERVACIONES, FECHA) VALUES (@MOTIVO, @MONTO, @OBSERVACIONES, @FECHA)"
...
Salir:
    ...
    Private Sub BtnEditar_Click(sender As Object, e As EventArgs) Handles BtnEditar.Click
        ...
        cmd.Connection = conn
        cmd.CommandType = CommandType.Text
        Sql = "UPDATE TABLAINGRESOS SET MOTIVO = @MOTIVO, MONTO = @MONTO, OBSERVACIONES = @OBSERVACIONES, FECHA = @FECHA WHERE ID = @ID"
        cmd.CommandText = Sql
        cmd.Parameters.Clear()
...
Salir:
    ...
    Private Sub BtnEliminar_Click(sender As Object, e As EventArgs) Handles BtnEliminar.Click
        ...
        If CodigoABuscar = 0 Then
            MsgBox("Seleccione un gasto o dato de la grilla para poder eliminarlo.", vbInformation, "SGB")
            Exit Sub
        End If

        If MsgBox("¿Desea realmente eliminarlo?", vbYesNo, "SGB") = vbYes Then
            Dim Sql As String = "DELETE FROM TABLAINGRESOS WHERE ID = @ID"
...
Salir:
    ...
    Private Sub BtnFiltrar_Click(sender As Object, e As EventArgs) Handles BtnFiltrar.Click
        ...
        Dim DAFiltro As New MySqlDataAdapter("SELECT * FROM TABLAINGRESOS WHERE FECHA BETWEEN @inicio AND @final", conn)
